...
//    }
    ...
    private int dfs(int[][] grid,boolean[][] visited, int x, int y){
        int sum = 1;
        visited[x][y] = true;
        if(x > 0 && grid[x-1][y] == 1 && !visited[x-1][y]) sum += dfs(grid, visited, x-1 ,y);
        if(x < m - 1 && grid[x+1][y] == 1 && !visited[x+1][y]) sum += dfs(grid, visited, x+1 , y);
        if(y > 0 && grid[x][y-1] == 1 && !visited[x][y-1]) sum += dfs(grid, visited, x ,y-1);
        if(y < n - 1 && grid[x][y+1] == 1 && !visited[x][y+1]) sum += dfs(grid, visited, x ,y+1);
        return sum;
    }
}
